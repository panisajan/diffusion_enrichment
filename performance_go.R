...
# Load function
source("rwr.R")
go <- read.csv("go_pathway.csv", header = TRUE)
...
for (k in 1:94) {
  ...
  pathway_id <- go$Term[k]
  gene_symbols <- strsplit(go$gene_name[k], ",\\s*")[[1]]
  ...
  write.csv(results, paste0(pathway_id_no_colon, "_go_5method.csv"), row.names = FALSE)
}
